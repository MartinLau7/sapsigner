...
func (e *Emulator) FairPlaySAPSign(ctxRef *definitions.FPSAPContextOpaqueRef, iBuf []byte) ([]byte, error) {
	iLen := len(iBuf)
	if iLen > math.MaxUint32 {
		return nil, fmt.Errorf("FairPlaySAPSign: input buffer too large")
	}
	...
	funcAddr := e.commercekitO.SymbolAddress(library.SymbolFairPlaySAPSign)
	...
	if rax != 0 {
		return nil, fmt.Errorf("FairPlaySAPSign: %d", int32(rax))
	}
